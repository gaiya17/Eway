...
interface TeacherTutorialDetailsPageProps {
  tutorialId: string;
  onLogout?: () => void;
  onNavigate?: (page: string, data?: any) => void;
}

export function TeacherTutorialDetailsPage({
  tutorialId,
  onLogout,
  onNavigate,
}: TeacherTutorialDetailsPageProps) {
  ...
  const [addMethod, setAddMethod] = useState<'pdf' | 'video' | 'link'>('video');
  ...
  const handleAddContent = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!tutorialId || tutorialId === 'undefined') {
      alert('Error: No tutorial selected. Please go back and select a tutorial.');
      return;
    }
    setIsUploading(true);
    try {
      ...
      await apiClient.post(`/free-tutorials/${tutorialId}/contents`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });

      alert('Content added successfully!');
      setIsAddModalOpen(false);
      setNewContent({ name: '', url: '' });
  ...
  const handleDeleteContent = async (contentId: string) => {
    ...
    try {
      await apiClient.delete(`/free-tutorials/contents/${contentId}`);
  ...
  // Guard: No valid tutorialId — show error state
  if (!tutorialId || tutorialId === 'undefined') {
    return (
      <DashboardLayout
      ...
      >
        <div className="flex flex-col items-center justify-center py-40 text-center">
          ...
          <p className="text-white/50 mb-8">Could not load tutorial details. Please go back and select a tutorial again.</p>
          <button
            onClick={() => onNavigate?.('teacher-tutorials')}
  ...
  return (
    ...
    >
      <div className="space-y-8">
        ...
        <div>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="flex gap-6 items-start">
              <div className="w-24 h-24 rounded-2xl overflow-hidden border border-white/10 shrink-0 shadow-[0_0_20px_rgba(239,68,68,0.2)]">
                <img 
              ...
              <div>
                <h1 className="text-3xl font-bold text-white mb-2 uppercase">{tutorial?.title}</h1>
                <div className="flex items-center gap-4">
                  <span className="px-3 py-1 rounded-full bg-red-500/20 text-red-400 text-xs font-bold border border-red-500/30 uppercase tracking-widest leading-none">
                    {tutorial?.level}
                  </span>
                  <span className="text-white/40">•</span>
                  <span className="text-white/60 text-sm font-semibold">{tutorial?.subject}</span>
                  <span className="text-white/40">•</span>
        ...
        {/* Content List */}
        <GlassCard className="p-0 overflow-hidden text-left">
          <div className="p-6 border-b border-white/10 flex items-center justify-between">
            <div>
              <h2 className="text-xl font-bold text-white">Tutorial Material</h2>
              <p className="text-white/40 text-sm">Videos and Resources included in this tutorial</p>
            </div>
            <div className="px-4 py-2 rounded-lg bg-red-500/10 border border-red-500/20 text-red-400 text-xs font-bold flex items-center gap-2">
               <Youtube size={14} /> FREE ACCESS
            </div>
          </div>
          ...
          <div className="divide-y divide-white/10">
            {isLoading ? (
              ...
              <div className="p-20 text-center">
                <Video size={64} className="mx-auto text-white/5 mb-6" />
                <h3 className="text-white font-bold text-lg mb-2">No Material Yet</h3>
                <p className="text-white/40">Start adding video lessons or PDF resources.</p>
              </div>
            ) : (
              contents.map((item, index) => (
                <div key={item.id} className="p-4 flex items-center gap-4 group hover:bg-white/5 transition-colors">
                  ...
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${
                    ...
                    item.file_type === 'video' ? 'bg-red-500/10 text-red-400' :
                  ...
                  <div className="flex-1 min-w-0 text-left">
                    ...
                    <p className="text-white/40 text-[10px] truncate max-w-md">{item.file_url}</p>
                  </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <GlassCard className="w-full max-w-lg">
            <div className="flex items-center justify-between p-6 border-b border-white/10">
              <h2 className="text-2xl font-bold text-white">Add Tutorial Material</h2>
              <button
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-white/60 text-sm font-semibold mb-2 block text-left">Item Name</label>
                  <input
                    type="text"
                    required
                    ...
                    placeholder="e.g. Video Lesson 1"
                  />
                </div>

                {addMethod === 'link' ? (
                  <div>
                    <label className="text-white/60 text-sm font-semibold mb-2 block text-left">Link URL (YouTube/External)</label>
                    <input
                      type="url"
                      required
                  ...
                  <div>
                    <label className="text-white/60 text-sm font-semibold mb-2 block text-left">Upload {addMethod.toUpperCase()}</label>
                    <input 
                      ref={fileInputRef} 
                      type="file" 
                    ...
                    <div 
                      ...
                      className={`w-full py-8 border-2 border-dashed rounded-2xl flex flex-col items-center justify-center cursor-pointer transition-all ${
                        selectedFile ? 'border-green-500/50 bg-green-500/5' : 'border-white/10 hover:border-red-500/50 bg-white/5 hover:bg-white/10'
                      }`}
                    >
                      {selectedFile ? (
                        <>
